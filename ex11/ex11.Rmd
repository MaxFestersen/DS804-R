...
```{r kNN preproccesing, warning=FALSE, echo=FALSE}
set.seed(42069)
#getting data ready
...
compund_train <- knn_compound_norm #all of data
compund_test <- knn_compound_norm #all of data
compund_target_category <- compound.df[,3]#all data
compund_test_category <- compound.df[,3]#all data

#using acutally training data and test data
compound_train <- knn_compound_norm[ran,]
compound_test <- knn_compound_norm[-ran,] #rest of data
compound_target_category <- compound.df[ran,3]
compound_test_category <- compound.df[-ran,3]
...
predictions <- knn(compound_train,compund_test,cl=compound_target_category,use.all = TRUE ,k=19)
```


```{r kNN confussion matrix/predictions, warning=FALSE, echo=FALSE}
#confussion matrix
tab <- table(predictions,iris_test_category)
tab
...
knn_traing_error <- calc_error_rate(predicted.value=predictions, true.value=compund_test)
knn_traing_error

#CrossTable
library(gmodels)
CrossTable(y=compund_test_category,x=pr,prop.chiq=FALSE)
```

```{r kNN optimal k based on test/training set, warning=FALSE, echo=FALSE}
#displaying the accuracy of the KNN algorithm given the K=i
i=1
k.optm=1
for (i in 1:40){
knn.mod <- knn(compund_train,compund_test,cl=compund_target_category,k=i)
k.optm[i] <- 100* sum(compund_test_category==knn.mod)/NROW(iris_test_category)
k=i
cat(k, '=',k.optm[i],'\n')
}
...
```{r kNN plot based test/training set, warning=FALSE, echo=FALSE}
#plot
plot.df = data.frame(compund_test, predicted = predictions)
...
```{r kNN 10 fold cross validation, warning=FALSE, echo=FALSE}
