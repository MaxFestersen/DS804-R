...
The compound data was chosen for... reasons.
...
* K-means (MacQueen): ~61%
* K-means (Forgy): ~~65%
* K-means (Hartigan-Wong): 80%
* KNN: ~90-99% (depending on risk of willingness to overfit)
* DBscan: ~77%
* OPTICS: ~93%
...
```{r 5.4.1 KNN preproccesing, warning=FALSE, echo=FALSE}
data <- data %>% 
  dplyr::select(class, date, sentiment)
set.seed(13355345) #leek speak
#getting data ready
ran_shrek <- sample(1:nrow(data.points), 0.80* nrow(data.points))
knn_compound_norm_shrek <- as.data.frame(lapply(data.points, nor))
norm.test.shrek <- cbind(knn_compound_norm_shrek, classNames)
names(norm.test.shrek) <- c("date", "sentiment", "class")

#using all data, test and training are the same
compound_train_shrek <- knn_compound_norm_shrek #all of data
compound_test_shrek <- knn_compound_norm_shrek #all of data
compound_target_category_shrek <- factor(classNames)#all data
compound_test_category_shrek <- factor(classNames)#all data

#using actually training data and test data
compound_train_shrek <- knn_compound_norm_shrek[ran_shrek,] #train data values
compound_test_shrek <- knn_compound_norm_shrek[-ran_shrek,] #test data values
compound_target_category_shrek <- factor(data[ran_shrek,1]$class) #train data category
compound_test_category_shrek <- factor(data[-ran_shrek,1]$class) #test data category

#when K=1: potential underfit
#when K approaches total number of rows:potential overfit
# K should be an odd number
#so we should cross validate
sqrt(length(classNames))
#KNN algorithm where key is equal to square root of total number of rows
predictions_shrek <- knn(compound_train_shrek,compound_test_shrek,cl=compound_target_category_shrek,use.all = TRUE, k=12)

```

confussion matrix of KNN with K=19 and a 80% training set, 20% test set

```{r 5.4.2 KNN confussion matrix/predictions, warning=FALSE, echo=FALSE}
#confussion matrix
set.seed(696969) # nice nice nice
tab_shrek <- table(predictions_shrek, compound_test_category_shrek)
#tab_shrek
...
#confusion Matrix with accuracy
confusionMatrix(predictions_shrek, compound_test_category_shrek)

#CrossTable
library(gmodels)
#not sure we need this
#CrossTable(y=compound_test_category_shrek,x=predictions_shrek,prop.chiq=FALSE)

#error rate
mean(compound_test_category_shrek != predictions_shrek)

```
plot that displaying the accuracy of the KNN algorithm given the K=i

```{r 5.4.3 KNN optimal k based on test/training set, warning=FALSE, echo=FALSE}
#displaying the accuracy of the KNN algorithm given the K=i

i <- 1
k.optm <- 1
for (i in 1:40){
  knn.mod <- knn(train=compound_train_shrek, test = compound_test_shrek, cl=compound_target_category_shrek, k=i)
  k.optm[i] <- 100* sum(compound_test_category_shrek == knn.mod)/NROW(compound_test_category_shrek)
  k=i
  # cat(k, '=',k.optm[i],'\n')
}
...
```{r 5.4.4 KNN plot based test/training set, warning=FALSE, echo=FALSE}
#plot
set.seed(999) # NEIN NEIN NEIN

plot.df_shrek = data.frame(compound_test_shrek, predicted = predictions_shrek)
plot.df_shrek1 = data.frame(x = plot.df_shrek$sentiment, 
                      y = plot.df_shrek$date, 
                      predicted = plot.df_shrek$predicted)

boundary_shrek = ddply(plot.df_shrek1, .variables = "predicted", .fun = find_hull)

border_plot <- ggplot(plot.df_shrek, aes(sentiment, date, color = predicted, fill = predicted)) +
  geom_point(size = 2)
...
#plot with border
border_plot + geom_polygon(data = boundary_shrek, aes(x,y), alpha = 0.5)
...
```{r 5.4.5 KNN 10 fold cross validation, warning=FALSE, echo=FALSE}

set.seed(104) # Vi må vi're

trControl_shrek <- trainControl(method  = "cv",
                          number  = 10)

cross_fit_shrek <- train(class ~ .,
             method     = "knn",
             tuneGrid   = expand.grid(k = 1:30),
             trControl  = trControl_shrek,
             metric     = "Accuracy",
             data       = norm.test.shrek)

confusionMatrix(cross_fit_shrek)
#Cross validation best K as a plot
plot(cross_fit_shrek)
#Cross validation best K
cross_fit_shrek
#plot of ROC(repeated Cross-validation)
plot(cross_fit_shrek, print.thres = 0.5, type="S")

```
Cross validation prediction tested on the dataset(remember, Kappe is the expected accuracy)

```{r 5.4.5 KNN 10 fold cross validation_2, warning=FALSE, echo=FALSE}
set.seed(100440)


knnPredict_shrek <- predict(cross_fit_shrek, newdata = norm.test.shrek )
confusionMatrix(knnPredict_shrek, factor(data$class))
...
```{r Silhouette Coefficient Shrek, warning=FALSE, echo=FALSE}
set.seed(42069)

# 4 clusters
c_forgy4 <- kmeans(data.points, 4, algorithm = "Forgy")
sil_res4 <- silhouette(c_forgy4$cluster, dist(data.points))
c_forgy4plot1 <- fviz_cluster(c_forgy4, data = data.points,
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw()
             )
c_forgy4plot2 <- fviz_silhouette(sil_res4)
grid.arrange(c_forgy4plot1, c_forgy4plot2, nrow =2, ncol = 1)

# 5 clusters
c_forgy5 <- kmeans(data.points, 5, algorithm = "Forgy")
sil_res5 <- silhouette(c_forgy5$cluster, dist(data.points))
c_forgy5plot1 <- fviz_cluster(c_forgy5, data = data.points,
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw()
             )
c_forgy5plot2 <- fviz_silhouette(sil_res5)
grid.arrange(c_forgy5plot1, c_forgy5plot2, nrow =2, ncol = 1)

# 6 clusters
c_forgy6 <- kmeans(data.points, 6, algorithm = "Forgy")
sil_res6 <- silhouette(c_forgy6$cluster, dist(data.points))
c_forgyplot1 <- fviz_cluster(c_forgy6, data = data.points,
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw()
             )
c_forgyplot2 <- fviz_silhouette(sil_res6)
grid.arrange(c_forgyplot1, c_forgyplot2, nrow =2, ncol = 1)



#Some other result
fviz_nbclust(data.points, kmeans, method = "silhouette") +
  labs(subtitle = "Silhouette method")
```


### 6 Results
All methods were usefull for the new dataset. 88-89% accuracy.

### 7 Clustering results
Movies with different release years, can with the initial reviews after it's release, be clustered by date and the sentiment in the reviews.
This could be useful for categorizing movies with  with sequals after each new entry.
