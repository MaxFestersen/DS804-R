...
compound.df <- compound.df %>% 
  ...
  mutate(class = ifelse(class == 5, "Libs", class)) %>% 
...
for the eyes and tounge, since they have a clear mean. We would expect it to
have some trouble with the mouth, the rain and the virus,
since the cluster-means overlap with other clusters.
```{r k-means-testing-methods, results = "hide", warning=FALSE, echo=FALSE}
set.seed(42069) # Set seed to remove randomness when replicating results
...
plot_stuff <- function(df, predicted) {
  ggplot(df, aes(x, y, shape = class, color = predicted)) +
    geom_point()
}

plot_stuff(norm.compound, norm.compound$macqueen)
plot_stuff(norm.compound, norm.compound$forgy)
plot_stuff(norm.compound, norm.compound$har_won)
```
Using the k-means algorithms: Macqueen, Loyd/Forg and Hartigan-Wong, we found
Hartigan-Wong was the most accurate, when looking at the accuracy of guesses,
and mean macro average. They are visualised above.
...
It is worht noting that Hartigan-Wong gets Right-eye more wrong with no clustering,
and has a harder time handling Rain and Corona-Virus,
using more clusteres there than the others.
