...
opt_clust_shrek <- optics(data.points, minPts = 7)
plot(opt_clust_shrek, sub = "minPts = 7")
```
...
```{r 3.3.1 Optics-order, warning=FALSE, echo=FALSE}
plot(data.points, col = "grey", main = "Clustering order", sub = "minPts = 7")
polygon(data.points[opt_clust_shrek$order,])
```


```{r 3.3.2 Optics-dbscan, warning=FALSE, echo=FALSE}
res_db_shrek <- extractDBSCAN(opt_clust_shrek, 1.55)
...
Xi method: (but it does'nt do that well) 

```{r 3.3.4 Optics-xi, warning=FALSE, echo=FALSE}
res_xi_shrek <- extractXi(opt_clust_shrek, xi = 0.058)
plot(res_xi_shrek, sub = paste0("Xi extraction steepness threshold = ", res_xi_shrek$xi))
```

When looking at the above reachability plot, it looks like most movies were identified, however, There are to many splits and many points which are not identified, i.e., classified as noise.
...
Looking at the above plot, The movies do create some meaningful clusters, but it does have some trouble with movies that are not Shrek. For Shrek is love. Shrek is life. 
