...
## 1. We use minPts = dim + 1. A larger value can also be used.
## 2. We inspect the  k-NN distance plot for k = minPts - 1
kNNdistplot(td, k = 6-1)

## Noise seems to start around a 5-NN distance of .17
...
```
DBscan has two parameters: minPts (Dimensionality + 1, or higher), and epsilon from Knn.
We choose an epsilon from the Knn Distance plot (0.17) and use a minPts from the dimensionality 6 + 1 = 7

DBScan suggests 5 clusters.

"Lips", "Tongue", "Corona" are pretty good.

#### Test 2
```{r 3.3.1 DBSCAN, echo=FALSE, warning=FALSE}
...
```{r}
# apply kmeans
Sil_km_res1 <- kmeans(td, 5, iter.max = 20, algorithm="MacQueen")
Sil_km_res2 <- kmeans(td, 6, iter.max = 20, algorithm="MacQueen")
Sil_km_res3 <- kmeans(td, 6, iter.max = 20, algorithm="MacQueen")
Sil_km_res4 <- kmeans(td, 7, iter.max = 20, algorithm="MacQueen")
Sil_km_res5 <- kmeans(td, 8, iter.max = 20, algorithm="MacQueen")


# calculate silhouette
sil_res1 <- silhouette(Sil_km_res1$cluster, dist(norm.compound))
sil_res2 <- silhouette(Sil_km_res2$cluster, dist(norm.compound))
sil_res3 <- silhouette(Sil_km_res3$cluster, dist(norm.compound))
sil_res4 <- silhouette(Sil_km_res4$cluster, dist(norm.compound))
sil_res5 <- silhouette(Sil_km_res5$cluster, dist(norm.compound))

# plot silhouette
fviz_silhouette(sil_res1)
fviz_silhouette(sil_res2)
fviz_silhouette(sil_res3)
fviz_silhouette(sil_res4)
fviz_silhouette(sil_res5)
```
