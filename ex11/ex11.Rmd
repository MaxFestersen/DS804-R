...
plot.df_shrek1 = data.frame(x = plot.df_shrek$y, 
                      y = plot.df_shrek$x, 
                      predicted = plot.df_shrek$predicted)

boundary_shrek = ddply(plot.df_shrek1, .variables = "predicted", .fun = find_hull)

border_plot<-ggplot(plot.df_shrek, aes(y, x, color = predicted, fill = predicted)) +
  geom_point(size = 2)
#turn the graph so it is facing the right way
border_plot<-border_plot+ coord_flip()
#plot without border
...
knnPredict <- predict(cross_fit,newdata = norm.test.shrek )
confusionMatrix(knnPredict,compound.df$class)
